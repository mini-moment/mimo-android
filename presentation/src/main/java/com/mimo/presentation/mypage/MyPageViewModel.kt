...
constructor(
    ...
    val _myPostList = MutableStateFlow<List<Post>>(emptyList())
    val myPostList : StateFlow<List<Post>> get() = _myPostList
    ...
    fun getMyPost() {
        viewModelScope.launch {
            postRepository.getMyPost().collectLatest { result ->
                when (result) {
                    is ApiResponse.Success -> {
                        setMyPostList(result.data)
                        Timber.d("나의 게시글 불러오기 성공! ${result.data}")
                    }

                    is ApiResponse.Error -> {
                        Timber.d("나의 게시글 불러오기 에러 발생! ${result.errorMessage}")
                    }
